...
def gen_chargerwallet_bin(nrf_dat: bytes, nrf_bin: bytes) -> bytes:
    buffer = io.BytesIO()
...
def main(input_file:str, output_file:str):
    ...
    chargerwallet_bin: bytes = gen_chargerwallet_bin(nrf_dat, nrf_bin)
    file_out = open(output_file, "wb")
    file_out.write(chargerwallet_bin)
    file_out.close()

    print(f'Created {output_file} ({len(chargerwallet_bin)} bytes)')
